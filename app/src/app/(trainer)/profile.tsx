import { useState } from 'react'; 
  ...
  import { Stack, router } from 'expo-router';                                         import { Colors } from '@/constants/colors';   
  ...
  const trainer = {                                                                  
    ...
    certifications: ['ISSA Certified PT', 'Nutrition Coach', 'CrossFit L2'],
    joinDate: 'March 2019',
    rating: 4.9,
    activeMembers: 18,
          ...
          <FadeInView delay={0}>
            <View style={styles.heroCard}>
              ...
              <View style={styles.specBadge}>
                <Text
  style={styles.specText}>{trainer.specialization.toUpperCase()}</Text>
              </View>
              <View style={styles.heroMetaRow}>
                <Text style={styles.heroMeta}>Since {trainer.joinDate}</Text>        
                <View style={styles.heroMetaDot} />
                <Text style={styles.heroMeta}>{trainer.experience} Experience</Text> 
              </View>
            </View>
          </FadeInView>
          ...
          <FadeInView delay={60}>
            <View style={styles.statsGrid}>
              {[
                { emoji: '👥', val: `${trainer.activeMembers}`,        label:        
  'MEMBERS' },
                { emoji: '🏋️', val: `${trainer.sessionsThisMonth}`,    label: 'THIS  MONTH' },
                { emoji: '📊', val: '1,480',                           label: 'TOTAL SESSIONS' },
                { emoji: '⭐', val: `${trainer.rating}`,               label:        
  'RATING' },
              ].map(s => (
          ...
          <FadeInView delay={160}>
            ...
            <View style={styles.card}>
              {trainer.certifications.map((cert, i) => (
                <View key={cert} style={[styles.certRow, i <
  trainer.certifications.length - 1 && styles.certRowBorder]}>
  ...
  const styles = StyleSheet.create({
    ...
    heroCard: {
      ...
      alignItems: 'center', gap: 8, borderWidth: 1, borderColor: Colors.accent +     
  '30',
    },
    avatarOuter: { position: 'relative', marginBottom: 4 },
    avatarInner: {
      width: 84, height: 84, borderRadius: 42,
      backgroundColor: Colors.bgElevated, borderWidth: 2, borderColor: Colors.accent,
      justifyContent: 'center', alignItems: 'center',
    },
    avatarText: { fontSize: 30, fontFamily: Fonts.condensedBold, color: Colors.accent
   },
    onlineDot: {
      position: 'absolute', bottom: 3, right: 3,
      width: 16, height: 16, borderRadius: 8,
      backgroundColor: Colors.green, borderWidth: 2, borderColor: Colors.bgCard,     
    },
    trainerName: { fontSize: 22, fontFamily: Fonts.condensedBold, color: Colors.text,
   letterSpacing: 0.5 },
    trainerRole: { fontSize: 12, fontFamily: Fonts.regular, color: Colors.textMuted  
  },
    specBadge: {
  ...
  letterSpacing: 1.2 },
    ...
    heroMeta: { fontSize: 12, fontFamily: Fonts.regular, color: Colors.textMuted },  
    heroMetaDot: { width: 3, height: 3, borderRadius: 2, backgroundColor:
  Colors.textMuted },
  ...
  letterSpacing: 1.2 },
    infoVal: { fontSize: 14, fontFamily: Fonts.medium, color: Colors.text, marginTop:
   2 },
    ...
    certDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: Colors.accent  
  },
    certText: { flex: 1, fontSize: 14, fontFamily: Fonts.medium, color: Colors.text  
  },
  ...
  textAlign: 'center' },
    ...
    toggleDesc: { fontSize: 11, fontFamily: Fonts.regular, color: Colors.textMuted,  
  marginTop: 2 },
    ...
    logoutBtn: {
      ...
      backgroundColor: Colors.red + '12', borderWidth: 1, borderColor: Colors.red +  
  '40',
    },
