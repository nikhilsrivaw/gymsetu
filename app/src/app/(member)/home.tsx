 import { useState, useCallback } from 'react'; 
  ...
  import { MaterialCommunityIcons } from '@expo/vector-icons';                         import { useSafeAreaInsets } from 'react-native-safe-area-context';                
  ...
  interface LbEntry { rank: number; member_id: string; name: string; days: number;   
  isMe: boolean; }
  ...
  'GOOD EVENING';
    ...
    useFocusEffect(useCallback(() => {
      ...
      async function load() {
        ...
        const [planRes, attMonthRes, attStreakRes, lbRes] = await Promise.all([      
          supabase
            .from('member_plans')
  ...
  86400000));
        ...
        const dateSet = new Set((attStreakRes.data ?? []).map((a: any) =>
  a.check_in_date));
        let s = 0;
        const d = new Date();
  ...
  d.setDate(d.getDate() - 1); }
        ...
        for (const row of (lbRes.data ?? [])) {
          ...
          if (!lbMap.has(key)) lbMap.set(key, { name: (row as any).members?.full_name
   ?? '?', days: 0 });
          lbMap.get(key)!.days++;
        }
        const lb: LbEntry[] = Array.from(lbMap.entries())
          .map(([id, v]) => ({ member_id: id, name: v.name, days: v.days, rank: 0,   
  isMe: id === m.id }))
          ...
          <View style={styles.memberCard}>
            ...
            <View style={styles.memberCardStats}>
              {[
                ...
                { val: `${Math.min(100, Math.round((daysThisMonth / 26) * 100))}%`,  
  label: 'GOAL' },
              ].map((s, i) => (
  ...
  styles.waterDropFilled]}>
            ...
            <View style={styles.waterBar}>
              <View style={[styles.waterFill, { width: `${(water / waterGoal) *      
  100}%` as any }]} />
            </View>
          </View>
        </FadeInView>
          ...
          <FadeInView delay={400}>
            ...
            <View style={styles.card}>
              {leaderboard.map((l, i) => (
                <View
                  key={l.member_id}
                  style={[styles.lbRow, l.isMe && styles.lbRowMe, i <
  leaderboard.length - 1 && styles.lbRowBorder]}
                >
  ...
  const styles = StyleSheet.create({
    ...
    menuBtn: { width: 40, height: 40, borderRadius: 10, backgroundColor:
  Colors.bgCard, justifyContent: 'center', alignItems: 'center', borderWidth: 1,     
  borderColor: Colors.border },
    greetSub: { fontFamily: Fonts.medium, fontSize: 9, color: Colors.textMuted,      
  letterSpacing: 1.2 },
    greetName: { fontFamily: Fonts.bold, fontSize: 20, color: Colors.text, marginTop:
   1 },
    streakBadge: { backgroundColor: Colors.accentMuted, borderRadius: 10,
  paddingHorizontal: 12, paddingVertical: 8, alignItems: 'center', flexDirection:    
  'row', gap: 6, borderWidth: 1, borderColor: Colors.accent + '30' },
    streakNum: { fontFamily: Fonts.condensedBold, fontSize: 24, color: Colors.accent 
  },
    streakLabel: { fontFamily: Fonts.medium, fontSize: 8, color: Colors.accent,      
  letterSpacing: 0.8, lineHeight: 11 },
    memberCard: { backgroundColor: Colors.bgCard, borderRadius: 20, padding: 20, gap:
   14, overflow: 'hidden', borderWidth: 1, borderColor: Colors.border },
    cardGlow: { position: 'absolute', top: -20, right: -20, width: 120, height: 120, 
  borderRadius: 60, backgroundColor: Colors.accentGlow },
    memberCardTop: { flexDirection: 'row', justifyContent: 'space-between',
  alignItems: 'flex-start' },
    cardMicroLabel: { fontFamily: Fonts.medium, fontSize: 9, color: Colors.textMuted,
   letterSpacing: 1.2, marginBottom: 4 },
    memberCardPlan: { fontFamily: Fonts.condensedBold, fontSize: 22, color:
  Colors.text, letterSpacing: 0.5 },
    expiryBadge: { backgroundColor: Colors.accentMuted, borderRadius: 12,
  paddingHorizontal: 12, paddingVertical: 8, alignItems: 'center' },
    expiryNum: { fontFamily: Fonts.condensedBold, fontSize: 26, color: Colors.accent 
  },
    expiryUnit: { fontFamily: Fonts.medium, fontSize: 8, color: Colors.accent,       
  letterSpacing: 0.8, textAlign: 'center' },
    planTrack: { height: 3, backgroundColor: Colors.border, borderRadius: 2,
  overflow: 'hidden' },
    ...
    mcStatLabel: { fontFamily: Fonts.medium, fontSize: 9, color: Colors.textMuted,   
  letterSpacing: 0.8 },
    qrCard: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor:  
  Colors.bgCard, borderRadius: 14, padding: 16, borderWidth: 1, borderColor:
  Colors.accent + '25' },
    qrIconWrap: { width: 48, height: 48, borderRadius: 12, backgroundColor:
  Colors.accentMuted, justifyContent: 'center', alignItems: 'center' },
    qrTitle: { fontFamily: Fonts.bold, fontSize: 13, color: Colors.text,
  letterSpacing: 0.5 },
    qrSub: { fontFamily: Fonts.regular, fontSize: 12, color: Colors.textMuted,       
  marginTop: 2 },
    qrArrow: { width: 32, height: 32, borderRadius: 8, backgroundColor:
  Colors.accentMuted, justifyContent: 'center', alignItems: 'center' },
    card: { backgroundColor: Colors.bgCard, borderRadius: 16, padding: 16, gap: 12,  
  borderWidth: 1, borderColor: Colors.border },
    cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 
  'center' },
    cardTitle: { fontFamily: Fonts.bold, fontSize: 11, color: Colors.textMuted,      
  letterSpacing: 1.2 },
    waterCount: {},
    ...
    waterDrop: { width: 34, height: 34, borderRadius: 8, backgroundColor:
  Colors.bgElevated, justifyContent: 'center', alignItems: 'center' },
    waterDropFilled: { backgroundColor: Colors.accentMuted },
    waterBar: { height: 3, backgroundColor: Colors.border, borderRadius: 2, overflow:
   'hidden' },
    waterFill: { height: 3, backgroundColor: Colors.accent, borderRadius: 2 },       
    sectionTitle: { fontFamily: Fonts.bold, fontSize: 10, color: Colors.textMuted,   
  letterSpacing: 1.5, marginTop: 4 },
    noticeCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 12,
  backgroundColor: Colors.bgCard, borderRadius: 12, padding: 14, borderWidth: 1,     
  borderColor: Colors.border, marginBottom: 6 },
    noticeDot: { width: 8, height: 8, borderRadius: 4, backgroundColor:
  Colors.accent, marginTop: 5 },
    noticeTitle: { fontFamily: Fonts.bold, fontSize: 13, color: Colors.text },       
    noticeBody: { fontFamily: Fonts.regular, fontSize: 12, color: Colors.textMuted,  
  marginTop: 2 },
    noticeTime: { fontFamily: Fonts.medium, fontSize: 10, color: Colors.textMuted }, 
    lbRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical:   
  10, paddingHorizontal: 4 },
    lbRowBorder: { borderBottomWidth: 1, borderBottomColor: Colors.border },
    lbRowMe: { backgroundColor: Colors.accentMuted, borderRadius: 10,
  paddingHorizontal: 10, marginHorizontal: -4 },
    lbRank: { fontFamily: Fonts.condensedBold, fontSize: 15, color: Colors.textMuted,
   width: 28 },
    lbName: { flex: 1, fontFamily: Fonts.medium, fontSize: 13, color: Colors.text }, 
    lbDays: { fontFamily: Fonts.condensedSemi, fontSize: 14, color: Colors.textMuted 
  },
  });