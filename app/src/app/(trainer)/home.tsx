  ...
  const todaySessions = [
    { id: 1, time: '6:00 AM',  member: 'Amit Singh',  type:        
  'Strength Training',   duration: '60 min', status: 'done' },     
    { id: 2, time: '7:30 AM',  member: 'Priya Nair',  type: 'Weight Loss',         duration: '45 min', status: 'done' },
    { id: 3, time: '5:00 PM',  member: 'Rahul Mehta', type: 'Muscle Gain',         duration: '60 min', status: 'upcoming' },        
    { id: 4, time: '6:30 PM',  member: 'Sneha Patel', type: 'Yoga &Flexibility',  duration: '50 min', status: 'upcoming' },        
  ];

  const alerts = [
    { id: 1, msg: 'Amit Singh missed 3 sessions this week',  color:
   Colors.orange },
    { id: 2, msg: 'Priya Nair reached her weight goal! 🎉',  color:
   Colors.green },
    { id: 3, msg: "Rahul Mehta's plan expires in 5 days",    color:
   Colors.red },
  ];
  ...
  const quickActions = [
    { label: 'Mark Attendance', emoji: '✅', color: Colors.green,  
   screen: 'attendance' },
    { label: 'Log Progress',    emoji: '📈', color: Colors.accent, 
   screen: 'progress-log' },
    { label: 'View Members',    emoji: '👥', color: Colors.orange, 
   screen: 'my-members' },
    { label: 'Workout Plans',   emoji: '📋', color: '#8B5CF6',     
  ...
  color={Colors.text} />
            ...
            <View style={styles.ratingBadge}>
              <Text
  style={styles.ratingVal}>{trainer.rating}</Text>
  ...
  Colors.accent + '40' }]}>
              <Text style={styles.statEmoji}>✅</Text>
              <Text style={styles.statVal}>{doneSessions}<Text     
  style={styles.statValSub}>/{todaySessions.length}</Text></Text>  
  ...
  SESSIONS</Text>
            <View style={styles.barChart}>
              {weekStats.map((w, i) => {
                const isToday = i === 4; // Friday = today mock    
                const fillPct = (w.sessions / MAX_SESSIONS) * 100; 
                return (
                  <View key={i} style={styles.barCol}>
                    <Text style={[styles.barVal, isToday && {      
  color: Colors.accent }]}>{w.sessions}</Text>
                    <View style={styles.barTrack}>
                      <View style={[styles.barFill, {
                        height: `${fillPct}%` as any,
                        backgroundColor: i === 6 ? Colors.border : 
  isToday ? Colors.accent : Colors.green,
                      }]} />
                    </View>
                    <Text style={[styles.barDay, isToday && {      
  color: Colors.accent }]}>{w.day}</Text>
                  </View>
                );
              })}
            </View>
  ...
  SESSIONS</Text>
            <View style={styles.upcomingPill}>
              <Text
  style={styles.upcomingPillText}>{upcomingSessions}
  UPCOMING</Text>
            </View>
          </View>
        </FadeInView>
  ...
  && styles.sessionCardDone]}>
              {/* Left color bar */}
              <View style={[styles.sessionBar, { backgroundColor:  
  s.status === 'done' ? Colors.green : Colors.accent }]} />        
              <View style={styles.sessionTimeCol}>
                <Text
  style={styles.sessionTimePrimary}>{s.time.split(' ')[0]}</Text>  
                <Text
  style={styles.sessionTimeSuffix}>{s.time.split(' ')[1]}</Text>   
              </View>
  ...
  s.status === 'done' ? Colors.green : Colors.accent }]} />        
        ...
        {alerts.map((a, i) => (
          <FadeInView key={a.id} delay={490 + i * 50}>
            <View style={[styles.alertCard, { borderLeftColor:     
  a.color }]}>
  ...
  alignItems: 'center',
    ...
    greetSub: { fontSize: 9, fontFamily: Fonts.bold, color:        
  Colors.textMuted, letterSpacing: 1.5 },
    greetName: { fontSize: 20, fontFamily: Fonts.condensedBold,    
  color: Colors.text, letterSpacing: 0.3, marginTop: 2 },
    ratingBadge: {
      flexDirection: 'row', alignItems: 'center', gap: 3,
      backgroundColor: Colors.green + '18', borderRadius: 20,      
      paddingHorizontal: 12, paddingVertical: 6,
      borderWidth: 1, borderColor: Colors.green + '30',
    },
    ratingVal: { fontSize: 15, fontFamily: Fonts.condensedBold,    
  color: Colors.green },
  ...
  color: Colors.text },
    statValSub: { fontSize: 15, fontFamily: Fonts.condensed, color:
   Colors.textMuted },
  ...
  6, height: 88 },
    ...
    barVal: { fontSize: 10, fontFamily: Fonts.bold, color:
  Colors.textMuted },
  ...
  Colors.accent, letterSpacing: 1.5 },
    upcomingPill: { backgroundColor: Colors.accentMuted,
  borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4,     
  borderWidth: 1, borderColor: Colors.accent + '30' },
    upcomingPillText: { fontSize: 9, fontFamily: Fonts.bold, color:
   Colors.accent, letterSpacing: 1 },
    ...
    sessionTimeCol: { width: 54, alignItems: 'center',
  paddingVertical: 14, gap: 1 },
    sessionTimePrimary: { fontSize: 14, fontFamily:
  Fonts.condensedBold, color: Colors.text },
    sessionTimeSuffix: { fontSize: 9, fontFamily: Fonts.bold,      
  color: Colors.textMuted, letterSpacing: 0.5 },
    sessionDivider: { width: 1, height: 34, backgroundColor:       
  Colors.border },
    sessionInfo: { flex: 1, paddingHorizontal: 12, paddingVertical:
   14 },
    sessionMember: { fontSize: 14, fontFamily: Fonts.bold, color:  
  Colors.text },
    sessionType: { fontSize: 11, fontFamily: Fonts.regular, color: 
  Colors.textMuted, marginTop: 2 },
    statusDot: { width: 7, height: 7, borderRadius: 4, marginRight:
   14 },
  ...
  14,
    ...
    alertMsg: { flex: 1, fontSize: 13, fontFamily: Fonts.regular,  
  color: Colors.textSub, lineHeight: 19 },
   ...
   marginTop: 10 },
    ...
    actionIconWrap: { width: 48, height: 48, borderRadius: 14,     
  justifyContent: 'center', alignItems: 'center' },
    actionEmoji: { fontSize: 24 },
    actionLabel: { fontSize: 12, fontFamily: Fonts.bold, color:    
