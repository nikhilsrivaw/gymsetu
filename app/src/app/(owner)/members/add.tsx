...
function NoteArea({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <View style={[na.outer, focused && na.outerFocused]}>
      {/* Subtle ruled lines — notebook feel */}
...
export default function AddMemberScreen() {
  ...
  const { profile, activeGymId, gymProfile } = useAuthStore();
  ...
  const gymId = activeGymId === 'all' ? profile?.gym_id : (activeGymId ?? profile?.gym_id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Assign plan immediately if one is selected
      if (selectedPlanId) {
        const plan = plans.find(p => p.id === selectedPlanId);
        if (plan) {
          // member_plans.member_id references members.id (NOT auth user / profile id)
          // Look up the actual members.id via user_id = credData.userId
          ...
          // recorded period is truthful. New joiner: starts today.
          ...
          if (!isExisting) {
            ...
            if (payError) console.warn('[add member] first payment record failed:', payError.message);
          } else if (planStatus === 'expired') {
            // Migrated member whose plan has already lapsed → mark them expired.
      ...
      <Modal visible={!!credentials} transparent animationType="fade">
        ...
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
            ...
            {/* Share on WhatsApp — sends credentials from the owner's own WhatsApp
                as a normal chat message (no Meta template needed). */}
            <AnimatedPressable
      ...
      >
        ...
        <FadeInView delay={200}>
          <SectionCard icon="card-account-details-outline" iconColor={Colors.green} title="MEMBERSHIP PLAN">
            {plans.length === 0 ? (
              <Text style={s.noPlanTxt}>No active plans found. Create plans first in the Plans tab.</Text>
            ) : (
            ...
            {selectedPlanId ? (
              <View style={s.payMethodWrap}>
                {/* Migration toggle — for gyms onboarding their existing members */}
