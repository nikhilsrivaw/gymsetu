 import { create } from 'zustand';                                                                             
  ...
  interface AuthState {
    ...
    signUp: (email: string, password: string, fullName: string, gymName: string) => Promise<{ error: string |   
  null }>;
  ...
  export const useAuthStore = create<AuthState>((set, get) => ({
    ...
    fetchProfile: async () => {
      // Use getUser() — always reflects the live auth state, not stale local session
    ...
    signIn: async (email, password) => {
      ...
      set({ session: data.session });   // set session first
    ...
    signUp: async (email, password, fullName, gymName) => {
      try {
        ...
        if (!authData.session) return { error: 'No session returned. Disable "Confirm email" in Supabase Auth settings.' };
        ...
        const { error: profileError } = await supabase
          .from('profiles')
          .insert({ id: userId, gym_id: gymData.id, role: 'gym_owner', full_name: fullName });
