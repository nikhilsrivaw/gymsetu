 import { useState, useCallback } from 'react';                                     
  ...
  interface PlanRow     { id: string; planName: string; startDate: string; endDate:  
  string; status: string; price: number; }
  interface PaymentRow  { id: string; date: string; planName: string; amount: number;
   method: string; }
  ...
  export default function MemberProfileScreen() {
    ...
    useFocusEffect(useCallback(() => {
      ...
      async function load() {
        ...
        const [memberRes, plansRes, paymentsRes, attRes, attCountRes] = await        
  Promise.all([
          supabase
            .from('members')
            .select('*')
        ...
        setPlans((plansRes.data ?? []).map((p: any) => ({
          ...
          startDate: new Date(p.start_date).toLocaleDateString('en-IN', { day:       
  '2-digit', month: 'short', year: 'numeric' }),
          endDate:   new Date(p.end_date).toLocaleDateString('en-IN',   { day:       
  '2-digit', month: 'short', year: 'numeric' }),
          status:    p.status,
  ...
  '2-digit', month: 'short', year: 'numeric' }),
        ...
        setAttendance((attRes.data ?? []).map((a: any) => ({
          id:   a.id,
          date: new Date(a.check_in_date).toLocaleDateString('en-IN', { day:
  '2-digit', month: 'short' }),
    ...
    if (loading || !member) {
      return (
        <>
          ...
          <View style={{ flex: 1, backgroundColor: Colors.bg, justifyContent:        
  ...
  2).toUpperCase();
   ...
   '2-digit', month: 'short', year: 'numeric' })
              ...
              {
                label: 'JOINED',
                value: new Date(member.join_date).toLocaleDateString('en-IN', { day: 
  '2-digit', month: 'short', year: 'numeric' }),
                emoji: '📅',
              },
   ...
   '—', emoji: '⚖️' },
              { label: 'GOAL',   value: member.goal ? (goalLabel[member.goal] ??     
  member.goal) : '—', emoji: '🎯' },
            ].map((row, i, arr) => (
  ...
  style={styles.planPrice}>₹{p.price.toLocaleString('en-IN')}</Text>
      ...
      if (activeTab === 'payments') {
        if (!payments.length) return <EmptyTab emoji="💳" text="No payments recorded 
  yet" />;
        return (
          <View style={styles.tabContent}>
            {payments.map((p, i) => (
              <View key={p.id} style={[styles.payItem, i < payments.length - 1 &&    
  ...
  style={styles.payAmount}>+₹{p.amount.toLocaleString('en-IN')}</Text>
      ...
      if (activeTab === 'attendance') {
        if (!attendance.length) return <EmptyTab emoji="📋" text="No attendance      
  records yet" />;
        return (
          <View style={styles.tabContent}>
            ...
            {attendance.map((a, i) => (
              <View key={a.id} style={[styles.attendRow, i < attendance.length - 1 &&
  ...
  Linking.openURL(`tel:${member.phone}`)}>
              ...
              <AnimatedPressable style={styles.actionBtn} scaleDown={0.94}
                onPress={() => member.phone &&
  Linking.openURL(`https://wa.me/${member.phone.replace(/\D/g, '')}`)}>
                <Text style={styles.actionEmoji}>💬</Text>
  ...
  params: { memberId: id, memberName: member.full_name } })}>
              ...
              <AnimatedPressable style={styles.actionBtn} scaleDown={0.94}
                onPress={() => router.push({ pathname: '/(owner)/members/edit',      
  params: { memberId: id } })}>
  ...
  styles.tabTextActive]}>{t.label}</Text>
  ...
  const styles = StyleSheet.create({
    ...
    avatarRing:  { width: 84, height: 84, borderRadius: 42, borderWidth: 2,
  justifyContent: 'center', alignItems: 'center' },
    avatarInner: { width: 72, height: 72, borderRadius: 36, justifyContent: 'center',
   alignItems: 'center' },
    avatarText:  { fontFamily: Fonts.condensedBold, fontSize: 26 },
    heroName:    { fontFamily: Fonts.condensedBold, fontSize: 28, color: Colors.text,
  ...
  paddingHorizontal: 12, paddingVertical: 5, borderRadius: 10, gap: 6 },
    ...
    actions:        { flexDirection: 'row', paddingHorizontal: 16, gap: 8,
  marginBottom: 20 },
    actionBtn:      { flex: 1, backgroundColor: Colors.bgCard, borderRadius: 12,     
  paddingVertical: 14, alignItems: 'center', gap: 5, borderWidth: 1, borderColor:    
  Colors.border },
  ...
  letterSpacing: 0.8 },
    ...
    tabBar:        { flexDirection: 'row', marginHorizontal: 16, backgroundColor:    
  Colors.bgCard, borderRadius: 12, padding: 4, marginBottom: 12, borderWidth: 1,     
  ...
  borderRadius: 1, backgroundColor: Colors.accent },
    ...
    contentCard: { marginHorizontal: 16, backgroundColor: Colors.bgCard,
  ...
  letterSpacing: 1, width: 60 },
    infoValue:     { flex: 1, fontFamily: Fonts.medium, fontSize: 13, color:
  Colors.text, textAlign: 'right' },

    tabContent: { padding: 12, gap: 8 },

    planItem:      { backgroundColor: Colors.bgElevated, borderRadius: 10, padding:  
  12, gap: 4, borderLeftWidth: 3 },
