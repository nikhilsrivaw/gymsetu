import {                                                                               View, Text, StyleSheet, Animated,                                                    Dimensions, ScrollView, TouchableOpacity,                                        
  ...
  const OWNER_FEATS: { icon: IconName; label: string; desc: string; color: string }[]
   = [
    ...
    { icon: 'cash-multiple',          label: 'Payments',    desc: 'Fees, dues &  history',          color: Colors.green  },
    { icon: 'chart-bar',              label: 'Analytics',   desc: 'Revenue & insights',            color: '#3B82F6'     },
    { icon: 'calendar-check-outline', label: 'Attendance',  desc: 'Daily check-ins', 
                color: Colors.orange },
  ...
  const MEMBER_FEATS: { icon: IconName; label: string; desc: string; color: string   
    ...
    { icon: 'clipboard-list-outline', label: 'Workouts',    desc: 'Log sets & reps', 
                color: Colors.accent },
    { icon: 'food-apple-outline',     label: 'Diet Plan',   desc: 'Meals & calories',
                color: Colors.green  },
    { icon: 'scale-bathroom',         label: 'BMI',         desc: 'Body mass index', 
                color: '#3B82F6'     },
    { icon: 'trending-up',            label: 'Progress',    desc: 'Weight measurements',         color: Colors.orange },
  ...
  const ROLES: {
    ...
    { key: 'owner',   label: 'GYM OWNER', sub: 'Run & manage your gym',       icon:  
  'crown-outline',   color: Colors.accent, route: '/(owner)/dashboard', perk:        
  'Members, payments & reports'    },
    { key: 'member',  label: 'MEMBER',    sub: 'Track your fitness journey',   icon: 
  'account-outline', color: Colors.green,  route: '/(member)/home',     perk:        
  'Workouts, diet plan & BMI'       },
    { key: 'trainer', label: 'TRAINER',   sub: 'Manage your clients',          icon: 
  'whistle-outline', color: '#3B82F6',     route: '/(trainer)/home',    perk: 'Client progress & sessions'     },
  ];
  ...
  export default function LandingScreen() {
    ...
    useEffect(() => {
      Animated.parallel([
        Animated.spring(logoS, { toValue: 1, tension: 55, friction: 7,
  useNativeDriver: true }),
      ...
      const t1 = setTimeout(() =>
        Animated.parallel([
          ...
          Animated.spring(titleY, { toValue: 0, tension: 60, friction: 9,
  useNativeDriver: true }),
        ]).start(), 300);
      ...
      const t3 = setTimeout(() =>
        Animated.parallel([
          ...
          Animated.spring(btnY, { toValue: 0, tension: 60, friction: 9,
  useNativeDriver: true }),
        ]).start(), 950);

      const pulse = (sv: Animated.Value, ov: Animated.Value, startO: number, delay:  
  number) => {
        setTimeout(() =>
          Animated.loop(
            Animated.sequence([
              Animated.parallel([
                Animated.timing(sv, { toValue: 2.6, duration: 2400, useNativeDriver: 
  true }),
                Animated.timing(ov, { toValue: 0,      duration: 2400,
  useNativeDriver: true }),
              ]),
              Animated.parallel([
                Animated.timing(sv, { toValue: 1,      duration: 0, useNativeDriver: 
  true }),
                Animated.timing(ov, { toValue: startO, duration: 0, useNativeDriver: 
  true }),
              ]),
            ])
          ).start(), delay);
    ...
    const navigate = (to: Screen) => {
      Animated.timing(outO, { toValue: 0, duration: 240, useNativeDriver: true       
  }).start(() => {
        setScreen(to);
        inO.setValue(0); inY.setValue(32);
        Animated.parallel([
          Animated.timing(inO, { toValue: 1, duration: 320, useNativeDriver: true }),
          Animated.spring(inY, { toValue: 0, tension: 55, friction: 9,
  useNativeDriver: true }),
      ...
      <Animated.View style={[s.fill, { opacity: outO }]}>
        ...
        <View style={s.splashCenter}>
          ...
          <View style={s.ringZone}>
            <Animated.View style={[s.ring, { width: 220, height: 220, borderRadius:  
  110, borderColor: Colors.accent, opacity: ring1O, transform: [{ scale: ring1S }]   
  }]} />
            <Animated.View style={[s.ring, { width: 168, height: 168, borderRadius:  
  84,  borderColor: Colors.accent, opacity: ring2O, transform: [{ scale: ring2S }]   
  }]} />
            <Animated.View style={[s.ring, { width: 124, height: 124, borderRadius:  
  62,  borderColor: Colors.orange, opacity: ring3O, transform: [{ scale: ring3S }]   
  }]} />
            <Animated.View style={[s.logoBubble, { opacity: logoO, transform: [{     
  scale: logoS }] }]}>
   ...
   />
          ...
          <Animated.View style={[{ alignItems: 'center', gap: 8 }, { opacity: titleO,
   transform: [{ translateY: titleY }] }]}>
            <Text style={s.appName}>GYMSETU</Text>
            <View style={s.divRow}>
              <View style={s.divLine} /><Text style={s.divText}>THE COMPLETE GYM     
  OS</Text><View style={s.divLine} />
            </View>
          </Animated.View>
  ...
  business — all in one app.
            </Text>
            <View style={s.statsRow}>
              {[{ v: '500+', l: 'GYMS' }, { v: '50K+', l: 'MEMBERS' }, { v: '4.9★',  
  l: 'RATING' }].map((st, i) => (
        ...
        <Animated.View style={[s.splashBottom, { opacity: btnO, transform: [{        
  translateY: btnY }] }]}>
          <Text style={s.madeIn}>🇮🇳  Proudly built for Indian gyms</Text>
          <TouchableOpacity style={s.ctaBtn} activeOpacity={0.82} onPress={() =>     
  navigate('features')}>
    ...
    const renderFeatures = () => (
      <Animated.View style={[s.fill, { opacity: inO, transform: [{ translateY: inY }]
        ...
        <ScrollView style={s.fill} contentContainerStyle={s.featScroll}
  showsVerticalScrollIndicator={false}>
          ...
          <View style={s.sectionHeader}>
            <View style={[s.sectionPill, { backgroundColor: Colors.accent + '18' }]}>
              <MaterialCommunityIcons name="crown-outline" size={13}
  color={Colors.accent} />
              <Text style={[s.sectionPillText, { color: Colors.accent }]}>FOR GYM    
  OWNERS</Text>
            </View>
          </View>
  ...
  color={Colors.green} />
              <Text style={[s.sectionPillText, { color: Colors.green }]}>FOR MEMBERS 
  & TRAINERS</Text>
            </View>
          </View>
          <View style={s.cardGrid}>
            ...
            <View style={s.whyBody}>
              ...
              {[
                ...
                <View key={w.t} style={s.whyRow}>
                  <MaterialCommunityIcons name={w.icon} size={14}
  color={Colors.accent} />
    ...
    const renderRoles = () => (
      <Animated.View style={[s.fill, { opacity: inO, transform: [{ translateY: inY }]
        ...
        <View style={s.rolesHeader}>
          ...
          <Text style={s.rolesSub}>Tap your role to enter the app</Text>
        </View>
  ...
  color={role.color} />
        ...
        <View style={s.rolesFooter}>
          <View style={s.authRow}>
            <TouchableOpacity style={s.authBtn} activeOpacity={0.8} onPress={() => router.push('/(auth)/login' as any)}>
              <MaterialCommunityIcons name="login" size={15} color={Colors.accent} />
              <Text style={s.authBtnText}>LOG IN</Text>
            </TouchableOpacity>
            <View style={s.authDivider} />
            <TouchableOpacity style={[s.authBtn, { borderColor: Colors.green + '40' }]} activeOpacity={0.8} onPress={() => router.push('/(auth)/register' as any)}>
              <MaterialCommunityIcons name="account-plus-outline" size={15} color={Colors.green} />
              <Text style={[s.authBtnText, { color: Colors.green }]}>SIGN UP</Text>
            </TouchableOpacity>
          </View>
  ...
  const s = StyleSheet.create({
    ...
    bgGlow1: { position: 'absolute', top: -80,  right: -80,  width: 260, height: 260,
   borderRadius: 130, backgroundColor: 'rgba(232,80,26,0.16)' },
    bgGlow2: { position: 'absolute', bottom: 180, left: -90, width: 220, height: 220,
   borderRadius: 110, backgroundColor: 'rgba(232,80,26,0.09)' },

    /* ── Splash ── */
    splashCenter: { flex: 1, alignItems: 'center', justifyContent: 'center',
  paddingHorizontal: 28, gap: 26 },
    ringZone:  { width: 220, height: 220, justifyContent: 'center', alignItems:      
  'center' },
    ...
    appName:  { fontFamily: Fonts.condensedBold, fontSize: 52, color: Colors.text,   
  letterSpacing: 10 },
    divRow:   { flexDirection: 'row', alignItems: 'center', gap: 10, alignSelf:      
  'stretch' },
    divLine:  { flex: 1, height: 1, backgroundColor: Colors.accent + '35' },
    divText:  { fontFamily: Fonts.bold, fontSize: 10, color: Colors.accent,
  letterSpacing: 2.5 },
    splashBody: { alignSelf: 'stretch', gap: 16 },
    splashDesc: { fontFamily: Fonts.regular, fontSize: 14, color: Colors.textMuted,  
  textAlign: 'center', lineHeight: 22 },
    statsRow:  { flexDirection: 'row', backgroundColor: Colors.bgCard, borderRadius: 
  16, borderWidth: 1, borderColor: Colors.border, paddingVertical: 14 },
    ...
    statLbl:   { fontFamily: Fonts.bold, fontSize: 8, color: Colors.textMuted,       
  letterSpacing: 1 },
    splashBottom: { paddingHorizontal: 24, paddingBottom: 10, gap: 12 },
    madeIn:    { fontFamily: Fonts.medium, fontSize: 12, color: Colors.textMuted,    
  textAlign: 'center' },

    /* shared */
    ctaBtn:     { flexDirection: 'row', alignItems: 'center', justifyContent:        
  'center', gap: 10, backgroundColor: Colors.accent, borderRadius: 16,
  paddingVertical: 17 },
    ctaBtnText: { fontFamily: Fonts.bold, fontSize: 15, color: Colors.bg,
  letterSpacing: 2 },
    skipText:   { fontFamily: Fonts.regular, fontSize: 12, color: Colors.textMuted,  
  textAlign: 'center' },

    /* ── Features ── */
    featScroll: { paddingHorizontal: 16, paddingTop: 16, paddingBottom: 8, gap: 10 },

    featHero:     { backgroundColor: Colors.bgCard, borderRadius: 20, padding: 20,   
  gap: 6, borderWidth: 1, borderColor: Colors.accent + '25', overflow: 'hidden',     
  marginBottom: 4 },
    featHeroGlow: { position: 'absolute', top: -28, right: -28, width: 110, height:  
  110, borderRadius: 55, backgroundColor: 'rgba(232,80,26,0.30)' },
    featMicro:    { fontFamily: Fonts.medium, fontSize: 9, color: Colors.accent,     
  letterSpacing: 1.8 },
    featTitle:    { fontFamily: Fonts.condensedBold, fontSize: 30, color:
  Colors.text, letterSpacing: 0.5 },
    featSub:      { fontFamily: Fonts.regular, fontSize: 12, color: Colors.textMuted 
  },

    sectionHeader: { marginBottom: 4 },
    sectionPill:   { flexDirection: 'row', alignItems: 'center', gap: 7, alignSelf:  
  'flex-start', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8 },       
    sectionPillText:{ fontFamily: Fonts.bold, fontSize: 10, letterSpacing: 1 },      

    cardGrid:     { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
    featCard:     { width: CARD_W, backgroundColor: Colors.bgCard, borderRadius: 16, 
  borderWidth: 1, overflow: 'hidden' },
    featCardTop:  { padding: 14, flexDirection: 'row', justifyContent:
  'space-between', alignItems: 'flex-start' },
    featCardIcon: { width: 44, height: 44, borderRadius: 12, justifyContent:
  'center', alignItems: 'center' },
    ...
    featCardDesc: { fontFamily: Fonts.regular, fontSize: 11, color: Colors.textMuted,
   lineHeight: 16 },

    whyCard:  { flexDirection: 'row', backgroundColor: Colors.bgCard, borderRadius:  
  16, borderWidth: 1, borderColor: Colors.accent + '25', overflow: 'hidden',
  marginTop: 6 },
    whyBar:   { width: 4, backgroundColor: Colors.accent },
    whyBody:  { flex: 1, padding: 16, gap: 10 },
    whyTitle: { fontFamily: Fonts.bold, fontSize: 11, color: Colors.accent,
  letterSpacing: 1.5, marginBottom: 2 },
    whyRow:   { flexDirection: 'row', alignItems: 'center', gap: 10 },
    whyText:  { fontFamily: Fonts.regular, fontSize: 12, color: Colors.textMuted,    
  flex: 1 },

    featBottom: { paddingHorizontal: 16, paddingTop: 12, paddingBottom: 10, gap: 10, 
  borderTopWidth: 1, borderTopColor: Colors.border, backgroundColor: Colors.bg },    

    /* ── Roles ── */
    rolesHeader: { paddingTop: 32, paddingHorizontal: 24, paddingBottom: 20, gap: 4  
  },
    rolesMicro:  { fontFamily: Fonts.medium, fontSize: 10, color: Colors.accent,     
  letterSpacing: 2.5 },
    rolesTitle:  { fontFamily: Fonts.condensedBold, fontSize: 46, color: Colors.text,
   letterSpacing: 2 },
    ...
    roleCard:       { flexDirection: 'row', alignItems: 'center', backgroundColor:   
  Colors.bgCard, borderRadius: 20, borderWidth: 1.5, overflow: 'hidden',
  paddingRight: 16, minHeight: 96 },
    roleBar:        { width: 6, alignSelf: 'stretch' },
    roleIconBubble: { width: 58, height: 58, borderRadius: 16, justifyContent:       
  'center', alignItems: 'center', borderWidth: 1.5, marginHorizontal: 16 },
    roleTextBlock:  { flex: 1, gap: 4, paddingVertical: 18 },
    roleLabel:      { fontFamily: Fonts.condensedBold, fontSize: 24, letterSpacing:  
  0.5 },
    roleSub:        { fontFamily: Fonts.regular, fontSize: 12, color:
  Colors.textMuted },
    rolePerkRow:    { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 
  2 },
    rolePerkText:   { fontFamily: Fonts.medium, fontSize: 10, color: Colors.textMuted
   },
    roleArrow:      { width: 42, height: 42, borderRadius: 13, justifyContent:       
  'center', alignItems: 'center' },
    ...
    authRow:     { flexDirection: 'row', gap: 10, alignSelf: 'stretch', paddingHorizontal: 16 },
    authBtn:     { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7, paddingVertical: 13, borderRadius: 13, backgroundColor: Colors.bgCard, borderWidth: 1, borderColor: Colors.accent + '40' },
    authBtnText: { fontFamily: Fonts.bold, fontSize: 13, color: Colors.accent, letterSpacing: 1 },
    authDivider: { width: 1, backgroundColor: Colors.border },
  });
