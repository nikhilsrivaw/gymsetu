import { useState, useCallback } from 'react';
  import { View, Text, StyleSheet, FlatList, TouchableOpacity, Alert, ActivityIndicator, Pressable } from
  'react-native';
  ...
  export default function AnnouncementsScreen() {
    ...
    const fetchAnnouncements = useCallback(async (active = true) => {
      ...
      // Fetch gym name for the active gym
      const nameGymId = activeGymId === 'all' ? mainGymId : (activeGymId ?? mainGymId);
      const { data: gym } = await supabase.from('gyms').select('name').eq('id', nameGymId).single();
    ...
    const handleAIWrite = async () => {
      if (!title.trim()) { Alert.alert('Add a title first', 'Enter a title so AI knows what to write about'); return;
   }
      setAiLoading(true);
      try {
    ...
    const handleAISocialPost = async () => {
      if (!title.trim()) { Alert.alert('Add a title first', 'Enter a title so AI can generate a social post');       
  return; }
      ...
      <>
        ...
        <View style={styles.container}>
          ...
          <Portal>
            <Modal
              visible={show}
              onDismiss={() => { setShow(false); resetForm(); }}
            ...
            >
              ...
              <View style={styles.modalForm}>
                ...
                <View>
                  <TextInput
                    label="Message" value={body} onChangeText={setBody}
                  ...
                  <View style={styles.aiBtnRow}>
                    ...
                    <TouchableOpacity
                      style={[styles.aiBtn, { borderColor: '#EC4899' + '40', backgroundColor: '#EC4899' + '15' }]}   
                      onPress={handleAISocialPost} disabled={socialLoading}
                    >
                      {socialLoading
                        ? <ActivityIndicator size="small" color="#EC4899" />
                        : <Text style={[styles.aiBtnText, { color: '#EC4899' }]}>📱 Social</Text>}
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[styles.aiBtn, { borderColor: Colors.orange + '40', backgroundColor: Colors.orange +    
  '15' }]}
                      onPress={handleAIPromo} disabled={promoLoading}
                    >
                      {promoLoading
                        ? <ActivityIndicator size="small" color={Colors.orange} />
                        : <Text style={[styles.aiBtnText, { color: Colors.orange }]}>🎯 Promo</Text>}
                    </TouchableOpacity>
                  </View>
                </View>
                  ...
                  {categories.map(c => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Text style={[styles.catChipText, active && { color, fontFamily: Fonts.bold }]}>
                          {c.label}
                        </Text>
                      </AnimatedPressable>
                    );
  ...
  const styles = StyleSheet.create({
    ...
    heroGlow: {
      position: 'absolute', top: -30, left: -20, width: 110, height: 110,
      borderRadius: 55, backgroundColor: Colors.accentGlow,
    },
    ...
    emptyDesc:  { fontFamily: Fonts.regular, fontSize: 13, color: Colors.textMuted, marginTop: 4, textAlign: 'center'
   },
  ...
  marginTop: 20 },
    ...
    aiBtn: {
      paddingHorizontal: 12, paddingVertical: 7, backgroundColor: Colors.accentMuted,
