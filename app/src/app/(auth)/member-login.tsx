 import { useState } from 'react';
  ...
  export default function MemberLoginScreen() {
    ...
    const handleLogin = async () => {
      if (!memberId || !password) { Alert.alert('Error', 'Please fill in all fields'); return; }
      // GSM-A3K9X → gsm-a3k9x@gymsetu.app
      const email = memberId.trim().toLowerCase() + '@gymsetu.app';
      setLoading(true);
      const { error, role } = await signIn(email, password);
      setLoading(false);
      if (error) { Alert.alert('Login Failed', 'Invalid Member ID or password'); return; }
      if (role === 'member') router.replace('/(member)/home');
      else Alert.alert('Access Denied', 'This login is for members only.');
    };
  ...
  showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.header}>
            <View style={[styles.logoBubble, { backgroundColor: Colors.green + '18', borderColor: Colors.green +
   '40' }]}>
              <MaterialCommunityIcons name="account-outline" size={30} color={Colors.green} />
            </View>
            <Text style={[styles.roleTag, { color: Colors.green, backgroundColor: Colors.green + '18'
  }]}>MEMBER</Text>
            <Text style={styles.title}>Member Login</Text>
            <Text style={styles.sub}>Use the ID & password given by your gym</Text>
          </View>

          <View style={[styles.infoCard, { backgroundColor: Colors.green + '10', borderColor: Colors.green +    
  '25' }]}>
            <MaterialCommunityIcons name="information-outline" size={15} color={Colors.green} />
            <Text style={styles.infoText}>
              Your Member ID looks like: <Text style={[styles.infoCode, { color: Colors.green
  }]}>GSM-A3K9X</Text>
            </Text>
          </View>

          <View style={styles.form}>
            <TextInput label="Member ID" value={memberId} onChangeText={setMemberId} autoCapitalize="characters"
   {...ip} />
            <TextInput
              label="Password"
              value={password}
  ...
  setSecureText(!secureText)} color={Colors.textMuted} />}
            ...
            />
            <TouchableOpacity
              style={[styles.btn, { backgroundColor: Colors.green }, loading && { opacity: 0.6 }]}
